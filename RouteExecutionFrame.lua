...
local function CreateExecutionFrame()
    ...
    -- GPS navigator toggle button — sits immediately left of the close button
    local gpsToggleBtn = CreateFrame("Button", nil, headerBar, "BackdropTemplate")
    gpsToggleBtn:SetSize(24, 24)
    gpsToggleBtn:SetPoint("RIGHT", closeBtn, "LEFT", -2, 0)
    gpsToggleBtn:SetBackdrop({
        bgFile = "Interface\\Buttons\\WHITE8x8",
        tile = false
    })
    gpsToggleBtn:SetBackdropColor(0, 0, 0, 0)
    local gpsToggleIcon = gpsToggleBtn:CreateTexture(nil, "ARTWORK")
    gpsToggleIcon:SetTexture("Interface\\MINIMAP\\ROTATING-MINIMAPARROW")
    gpsToggleIcon:SetSize(16, 16)
    gpsToggleIcon:SetPoint("CENTER", gpsToggleBtn, "CENTER")
    gpsToggleIcon:SetVertexColor(0.5, 0.7, 0.9, 1)
    gpsToggleBtn:SetScript("OnEnter", function(self)
    ...
    gpsToggleBtn:SetScript("OnClick", function()
        local ref = addon.GPSNavigatorFrame
        if not ref then
            return
        end
        if ref:IsShown() then

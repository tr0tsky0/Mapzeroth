-- Edges.lua (Forever)
--
-- Flight edges sourced from InFlight's global_classic override table only
-- (a WoW Anniversary/Classic-Era addon's crowd-measured pre-Cataclysm
-- flight-time data), restricted to the flight masters already collected in
-- Nodes_Kalimdor.lua / Nodes_EasternKingdoms.lua and cross-referenced by
-- taxi nodeID against our own Forever dump.
--
-- Earlier drafts of this file fell back to InFlight's dense BASE table
-- (its own source comment calls it "post-Cataclysm defaults") for any pair
-- global_classic didn't cover — dropped that. Live testing confirmed
-- Forever's real flight network is sparse and neighbor-only (opening the
-- flight map at a location only ever shows a handful of REACHABLE
-- destinations, not "anywhere on the continent" the way the dense table
-- implies), so a fallback value from that table isn't a best-effort
-- estimate — it can encode a connection that doesn't exist in Forever's
-- actual, differently-shaped network at all. A missing edge is a safer
-- failure mode than a wrong one.
--
-- This means the graph below is smaller than earlier drafts (827 edges,
-- global_classic's real coverage of our node set) but every single one of
-- them is genuine pre-Cataclysm data, not modern-retail reachability.
--
-- `requirements.faction` comes from which faction's table in InFlight
-- actually owned that node — real per-faction network data, not a guess.
-- A handful of nodes (Gadgetzan, Nordrassil/Summit of Eternity, and
-- apparently Northpass Tower for two specific destinations) are shared by
-- both factions' tables.
--
-- SUPERSEDING ALL OF THIS: live verification via MapzerothDataTools'
-- TAXIMAP_OPENED probe (see /mzverified) is actively collecting Forever's
-- real, current network as nodes get discovered in-game. Every edge below
-- is a starting baseline, not ground truth — replace with live-verified
-- data as it comes in. Confirmed so far: Crossroads -> Orgrimmar,
-- Camp Taurajo -> {Orgrimmar, Crossroads}. No live Alliance data yet
-- (node/edge visibility is per-faction; needs an Alliance character).
--
-- Two edges (Northpass Tower -> Undercity / Light's Hope Chapel) appear
-- under BOTH factions' base tables despite those destinations otherwise
-- being Horde-only-owned nodes — unresolved oddity in the source data,
-- flagged rather than silently trusted. (Kept below since global_classic
-- happened to cover these two specifically — verify like anything else.)

...
1233 nodes written across 14 files.
...
- **Ids** follow Forever's `<KIND>_<PLACE>` convention (tools/modern_ids.py has the rules; the whole map is tools/modern_source/id_map.tsv): 690 flight masters are `TAXI_<realID>`, 204 dungeon and raid entrances are `INSTANCE_<NAME>` nodes with `kind = "instance"` and their `journal` id, and 130 transports had their kind moved to the front (`BORALUS_DOCK` -> `DOCK_BORALUS`).
  - 7 rename collision(s) (two old nodes would get the same id -- the first keeps it, the other takes the next rule, see tools/modern_ids.py):

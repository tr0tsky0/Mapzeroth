...
1238 nodes written across 14 files.
...
- **Ids** follow Forever's `<KIND>_<PLACE>` convention (tools/modern_ids.py has the rules; the whole map is tools/modern_source/id_map.tsv): 694 flight masters are `TAXI_<realID>`, 204 dungeon and raid entrances are `INSTANCE_<NAME>` nodes with `kind = "instance"` and their `journal` id, and 128 transports had their kind moved to the front (`BORALUS_DOCK` -> `DOCK_BORALUS`).

...
The planner chains legs over the points the player has found and takes FLIGHT_CHAIN_SAVING seconds off
each extra leg (a through-ticket doesn't land and take off again).
...
CHAIN_SAVING = 10               # seconds off per extra leg; keep equal to addon.FLIGHT_CHAIN_SAVING
...
def read_truth(names):
    """Routes the game reported through /mzroutes: a list of stop lists."""
    def node_id(text):
    ...
    for line in (SRC / "game_routes.txt").read_text(encoding="utf-8").splitlines():
        m = re.match(r"(.+?) -> (.+?): (\d+) leg\(s\): (.+)$", line.strip())
        if m:
    ...
    def shortest(source):
        ...
        while heap:
            ...
            for v, t in adjacency.get(u, []):
                nd = d + t - (CHAIN_SAVING if chained else 0)
...
def main():
    ...
    for (a, b, f), t in sorted(times.items(), key=lambda kv: (kv[0][2], int(kv[0][0][5:]), int(kv[0][1][5:]))):
        ...
        lines.append(f'    {{ from = "{a}", to = "{b}", method = "flight", cost = {t}, '

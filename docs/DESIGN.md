...
**Packaging consequence:** the plan is two `.toc` files, `Mapzeroth_Mainline.toc` and a Forever one, each listing only its own data folder. **Interim (2026-09-24, checked in game on the Forever beta):** Forever's client is the Mainline flavour, so it loads a `_Mainline` `.toc` too (with Modern's data, which is how the rebuild's Forever client ended up routing on Modern nodes), and the Forever suffix isn't known yet: `_Forever` is ignored. Forever's release should have a suffix of its own (the user expects one); until it's known, the rebuild uses **one `.toc`** instead, declaring both interfaces (`## Interface: 16001, 120100`) and listing both data folders; Constants.lua sets `addon.RULESET` from the interface version (the only reliable discriminator) and every data file skips itself unless it's this client's, with a guard right after its header: `if addon.RULESET ~= "forever" then return end`. Each client still compiles the other game's data files at login (the guard stops them running, not loading); Modern's Geometry.lua is the largest. **Revisit at Forever's release** (user's call: guards that a working suffix makes unnecessary shouldn't stay): once the suffix is known, go back to two `.toc` files and drop the guards, `addon.RULESET`, `tests/test_rulesets.lua` and the generators' guard line.

**Data layout follows the same split.** Rather than tagging individual node/edge/ability entries with a ruleset field, each ruleset gets its own data folder, whose files each carry their ruleset's guard line:
...
No filtering needed inside the engine — each file's guard *is* the ruleset gate (every generator writes it, and `tests/test_rulesets.lua` checks every data file the `.toc` lists carries its own). This also settles how ability/item data (class teleports, hearthstones, toys — which ones even exist pre-Cataclysm) varies by ruleset: it's just a different file, not a shared table with per-entry ruleset flags.
...
So the full explicit-edge list (`portal`, `teleport`, `ship`, `taxi`, `phaseswitch`, plain `walk`/`fly` where hand-authored, etc) is (`taxi` is a flight-master ticket, called `flight` until 2026-09-23 -- renamed so it can't be confused with `fly`, a player flying their own mount between two points):
...
- **Phasing** (your point #3): a phase-variant zone is two sibling `Zone` containers sharing a `phaseGroup` tag, linked to *each other* only by an explicit `phaseswitch` edge, while each still auto-connects normally to its actual geographic neighbors like any other zone.
  ...
  **As built (2026-09-24, review finding 3):** a side is a map art id, and each side container says which map's art shows it: `{ phaseGroup = "darkshore", phaseSide = 67, phaseMap = 62 }`. The generator names the groups after the Zidormi nodes; Teldrassil carries Darkshore's past art, so it is a Darkshore-past side without being nested. The search starts from the live sides (`World:LivePhases`, from `C_Map.GetMapArtID`). A group the client can't tell apart (past and present Tirisfal, Uldum and the Vale are two maps, each always showing its own art) is left out of the start state and treated as open: either side can be entered, and nothing is recorded. An edge can also carry `inPhase = { group, side }`, usable only on that side whatever its ends are (the Dark Portal goes to Outland in the past, Draenor in the present). A route takes at most one phase switch. That keeps a search to one extra state per known group instead of every combination.
...
```
Mapzeroth/
  Mapzeroth.toc               -- Interface 16001, 120100: loads Data/Forever/* and Data/Modern/*; each data file
                                   skips itself unless it is this client's (addon.RULESET, Constants.lua)
...
1. **Forever's `.toc` suffix, and CurseForge packaging** — at Forever's release, find the flavour suffix its client loads (not `_Forever` on the beta; Forever also loads `_Mainline`), then return to per-game `.toc` files and remove the interim ruleset guards (section 1). Check whether CurseForge's packager knows the flavour. Not urgent — check when ready to publish.
...
3. **Vulpera's Make Camp / Return to Camp (Modern only) is a third kind of anywhere-to-dynamic-spot ability**, distinct from both a fixed-destination Teleport/Item and the bound-inn `ctx.hearthNode` pair (real Hearthstone item, Astral Recall): the camp isn't a settlement, it's wherever the player last cast Make Camp, which nothing currently tracks. The old addon never modeled this either (`PlayerAbilities.lua`'s `RacialAbilities` table has just a comment: "Potentially add later: Vulpera - Make Camp / Return to Camp (hearthstone-like)"). Needs: an event listener that captures the player's position when Make Camp is cast (a new piece of live state, not derivable from any static data — comparable to how `Hearth.lua` figures out a bind from `PLAYER_ENTERING_WORLD`/settings, not a lookup table) into its own `ctx.campNode`-style field, separate from `ctx.hearthNode` since a character can have both a hearth bind and an active camp at once, in different places; then a `Return to Camp` entry in `GetKnownTeleports` shaped like the `Hearthstones` loop but reading that field instead. Not started.
4. **Modern's auto-generated `fly` edges need real per-zone data, not the blanket `fly = true` at the container root.** `TravelGraph.lua`'s fly-edge generator is an O(n²) pairwise check per continent (fine for Forever, which has none), and a live character hit "script ran too long" from it on 2026-09-23 — Eastern Kingdoms alone has ~200 outdoor nodes. Root-caused further the same night: the old shipped addon (`Mapzeroth\Mapzeroth\TravelGraph.lua`) has the identical O(n²) brute-force check with the identical `MAX_AUTO_EDGE_DISTANCE` post-hoc filter — the algorithm was never the difference. What the old addon does differently is build its whole graph (including this pass) exactly *once*, at file-load time (`injectAutoTraversalEdges(addon.TravelGraph)` runs at module top level, not inside a per-request function), then applies player-specific filtering during the Dijkstra search itself (`CheckEdgeRequirements` inside the search loop, not at graph-build time). The rebuild's `TravelGraph:Build(ctx)` used to redo the *entire* graph — geometry included — from scratch on every call, and got called at least twice per click (once to price the picker's sections, again to plan the chosen route). Fixed the same night, in two passes: `TravelGraph.lua` first split the ctx-independent geometry (walk/gate/fly edges — position-only) from the ctx-dependent parts (authored-edge requirements, ground/flight speed, known teleports) and cached the former, keyed on `World.generation` so a `World:Build()` invalidates it, instead of rebuilding it on every request. Then, since node/edge data only ever changes through a new version pushed to CurseForge (not per-session), that geometry doesn't need computing client-side at all: `/mzr dumpgeometry` (MapzerothDataTools' Dev.lua) computes it once in-game and exports it as `Data/<ruleset>/Geometry.lua` (`addon.Geometry` + `addon.GeometryMeta.nodeCount`), which `TravelGraph.lua` uses as-is when present and its recorded node count still matches what's loaded — no computation at all, not even once. A mismatched count (a forgotten regeneration after a data change) falls back to computing it live instead of silently serving stale edges. `Data/Modern/Geometry.lua` is currently an empty placeholder (`addon.Geometry` unset) — **needs an actual `/mzr dumpgeometry` run and paste before this is live**, otherwise Modern still pays the live (now-cached) cost same as before. `addon.MAX_FLY_BUCKET` (Constants.lua) is a precaution for the live fallback path specifically, and its necessity was never actually confirmed there either -- it was set from node-count reasoning alone, not a measurement, and the old shipped addon does the identical O(n^2) pass, uncapped, in one synchronous call at file-load time for 125k users with no reported issue. The real, confirmed cause of the original crash was geometry being rebuilt on *every* route (sometimes twice per click) and combined with a live Dijkstra search in the same synchronous call, not the raw one-time pass by itself -- both now moot: `/mzr dumpgeometry` (`TravelGraph:BuildFreshGeometry`) explicitly skips the cap (`buildStaticGeometry(true)`), since it's a deliberate, isolated, one-time call with no search sharing its execution and no gameplay impact if it turns out to be slow -- exactly the kind of case worth actually testing rather than guessing defensively about. The live fallback still applies the cap as a safety net, since an uncapped first-route-of-the-session landing unpredictably mid-gameplay is a worse place to find out. If a real dump completes fine (expected, per the old addon's own precedent), Eastern Kingdoms/Kalimdor/Draenor/Northrend get their fly edges in the shipped file and the cap's live-path relevance drops to "only matters if the dump was never run." Curated per-zone `fly` data (old-world Eastern Kingdoms/Kalimdor don't actually allow free flying without Pathfinder -- a gameplay-accuracy question, separate from performance) is still not started.

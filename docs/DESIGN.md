...
**Packaging consequence (corrected 2026-09-24, checked in game):** the plan here was two `.toc` files, `Mapzeroth_Mainline.toc` and `Mapzeroth_Forever.toc`. It doesn't work. Forever's client is the Mainline flavour, so it loads a `_Mainline` `.toc` too (with Modern's data, which is how the rebuild's Forever client ended up routing on Modern nodes), and it has no flavour suffix of its own: a `_Forever` `.toc` is ignored. So the client can't be told apart by `.toc` name. **One `.toc`** instead, declaring both interfaces (`## Interface: 16001, 120100`) and listing both data folders; Constants.lua sets `addon.RULESET` from the interface version (the only reliable discriminator) and every data file skips itself unless it's this client's, with a guard right after its header: `if addon.RULESET ~= "forever" then return end`. Each client still compiles the other game's data files at login (the guard stops them running, not loading); Modern's Geometry.lua is the largest.
...
1. **CurseForge packaging** — with one `.toc` declaring two interfaces, check how CurseForge classifies it (which game versions it lists the file under). Not urgent — check when ready to publish.

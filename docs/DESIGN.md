...
**Flight times are per direction.** InFlight records each direction separately and they really do differ (Stormwind to Ironforge 259 s, back 210 s), so both are authored. The graph generates the reverse of an authored edge only when the reverse isn't authored itself; an earlier version generated it anyway and took the cheaper, which showed a 357 s flight as 3m21s.
